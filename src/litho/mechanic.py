...
class MM(MechanicEquations):
    ...
    def z_rheo(self, z, Z0, Zi, Zm, Zb, serp_domain):
        ...
        #)
        #print('z_rheo_expand_dims_started')
        r = xr.full_like(z, np.nan).expand_dims(
            dim={'var': ['domain', 'n', 'A', 'H']}, axis=3
        ).copy()
        #print('z_rheo_expand_dims_finished')
        #uc_rheo, lc_rheo, lm_rheo, serp_rheo = [
        #    {key: rhe_data[str(rheo_id)][key] for key in ['n', 'A', 'H']}
        #    if rheo_id is not None else None
        #    for rheo_id in [self.uc, self.lc, self.lm, self.serp]
        #]
        #print('z_rheo_get_rheo_info_started')
        rheos = self.get_rheo_info()
        #print('z_rheo_get_rheo_info_finished')
        #print('z_rheo_xr_where_Zi')
        r = xr.where((0 <= z) & (z < Zi), [0] + list(rheos['uc'].values()), r)
        #print('z_rheo_xr_where_Zm')
        r = xr.where((Zi <= z) & (z < Zm), [1] + list(rheos['lc'].values()), r)
        #print('z_rheo_xr_where_Zb')
        r = xr.where((Zm <= z) & (z <= Zb), [2] + list(rheos['lm'].values()), r)
        #print('z_rheo_serp')
        if self.serp is not None:
            ...
            r = xr.where(
                ...
                [3] + list(flm_rheo.values()),
                r
            )
        #print('z_rheo_finished')
        return r
    ...
    def calc_ductile_yield_strength_envelope(
        ...
        Zb = -(Zb-Z0)*1.e3
        z = -(z-Z0)*1.e3
        #print('calculating_ductile_yse_started')
        ...
        #print('calculating_ductile_yse_finished')
        return dyse

    def calc_yield_strength_envelope(self, byse, dyse, cond='compression'):
        byse_t, byse_c = byse
    ...
    def calc_yield_depths(
        ...
        yield_depths = {
            ...
            'ductile_temps': ductile_yield_temps,
        }
        return yield_depths

    def calc_elastic_tuples(self, Z0, Zi, Zm, Zb, yield_depths, cond='compression'):
        tuples = {}
        for rheo_key in yield_depths['ductile']['rheo'].values:
            ...
            top_elastic = minimum(
                top_boundary,
                yield_depths['brittle'].loc[:, :, cond]
            )
            bottom_elastic = maximum(
                bottom_boundary,
    ...
    def calc_eet(self, elastic_tuples):
        ...
        return eet, share_moho, share_icd
