...
def earthquakes_setup():
    csn = pd.read_csv(
        data_dir/'earthquakes/CSN_2000_2018_C_SB30_BINS_plus_Maule.csv',
        sep=','
    )
    usgs = pd.read_csv(
        data_dir/'earthquakes/USGS_1900_2018_C_SB30_BINS.csv',
        sep=','
    )
    usgs['source'] = 'USGS'
    usgs['color'] = 'blue'
    csn['source'] = 'CSN'
    csn['color'] = 'black'
    eqs = pd.concat([usgs,csn], axis=0, ignore_index=True).dropna(axis=1)
    return csn, usgs, eqs
